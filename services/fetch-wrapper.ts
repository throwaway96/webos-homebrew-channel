...
const loadCertDir = require('@throwaway96/node-load-cert-dir').loadCertDir;
...
function createAgent(): https.Agent {
  const certs = loadCertDir('/etc/ssl/certs', {
    filterFunc: (certData: string) => certData !== certDoNotWant,
    logLevel: 0,
  });

  certs.push(certIsrgOld);
  certs.push(certIsrgNew);
  certs.push(certDstMod);
